...
class Winrate:
    ...
    def get_winratelist(self):
        with dbapi2.connect(self.cp) as connection:
            ...
            query = """SELECT year_id, years.title, drivers.name, teams.title
                    FROM Winrates LEFT JOIN years ON (year_id = years.id) 
    ...
    def delete_winrate(self, id):
        with dbapi2.connect(self.cp) as connection:
            cursor = connection.cursor()
            query = "DELETE FROM Winrates WHERE year_id = '%s'" % (id) 
            cursor.execute(query)
            connection.commit()
            return

    def add_winrate(self, nyear, ndriv, nteam):
        with dbapi2.connect(self.cp) as connection:
            cursor = connection.cursor()
            ...
            query = """INSERT INTO Winrates VALUES ('%s','%s','%s')""" %(nid,cid,tid)
            cursor.execute(query)

            connection.commit()
            return
            
    def update_winrate(self, oyear, nyear, ndriv, nteam):
        with dbapi2.connect(self.cp) as connection:           
            cursor = connection.cursor()
            ...
            query = "UPDATE Winrates SET year_id = '%s', driver_id='%s',team_id='%s' WHERE year_id = '%s'" %(nid,cid,tid,oid)
            cursor.execute(query)
            connection.commit()            
            
            return
            
    def search_winratelist(self, name):
        with dbapi2.connect(self.cp) as connection:
            cursor = connection.cursor()
